  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 overflow-y-auto px-6 lg:px-8 py-6 space-y-6 max-w-7xl mx-auto w-full">
        {/* Project Metrics Strip with Distinct Color Washes */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-4 rounded-[12px] bg-emerald-500/[0.05] dark:bg-emerald-500/[0.08] border border-emerald-500/25 shadow-sm specimen-chamfer space-y-1.5 flex flex-col justify-between">
            ...
            <div>
              <div className="text-xl font-bold font-display text-emerald-700 dark:text-emerald-400 tabular-nums">{activeProject.targetStartDate}</div>
            ...
            <div className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold pt-1 border-t border-emerald-500/20 flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" />
              <span>Flights & Camp Pre-Approved</span>
            </div>
          </div>

          <div className="p-4 rounded-[12px] bg-[#8A6D3B]/[0.06] dark:bg-[#d4c5a9]/[0.08] border border-[#8A6D3B]/30 shadow-sm specimen-chamfer space-y-1.5 flex flex-col justify-between">
            <div className="type-eyebrow text-[#8A6D3B] dark:text-[#d4c5a9] flex items-center justify-between">
              <span>Headcount Fulfillment</span>
              <span className="font-bold">{Math.round((activeProject.assignedCount / activeProject.requiredHeadcount) * 100)}%</span>
            </div>
            <div>
              <div className="text-xl font-bold font-display text-[#8A6D3B] dark:text-[#d4c5a9] tabular-nums">
                {activeProject.assignedCount} of {activeProject.requiredHeadcount} Filled
              </div>
              <div className="w-full bg-[#8A6D3B]/20 rounded-full h-1.5 mt-1.5 overflow-hidden">
                <div
            ...
            <div className="text-[11px] text-slate-500 dark:text-zinc-400 pt-1 border-t border-[#8A6D3B]/20">
              {activeProject.requiredHeadcount - activeProject.assignedCount} open headcount slots remaining
            </div>
          </div>

          <div className={cn(
            'p-4 rounded-[12px] shadow-sm specimen-chamfer space-y-1.5 flex flex-col justify-between border',
            activeProject.riskLevel === 'high'
              ? 'bg-rose-500/[0.05] dark:bg-rose-500/[0.08] border-rose-500/30'
              : activeProject.riskLevel === 'medium'
              ? 'bg-amber-500/[0.05] dark:bg-amber-500/[0.08] border-amber-500/30'
              : 'bg-emerald-500/[0.05] dark:bg-emerald-500/[0.08] border-emerald-500/30'
          )}>
            <div className={cn('type-eyebrow flex items-center justify-between', activeProject.riskLevel === 'high' ? 'text-rose-800 dark:text-rose-300' : 'text-amber-800 dark:text-amber-300')}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {activeProject.roles.map((r, idx) => {
              const isPiping = r.discipline.toLowerCase().includes('piping');
              const isMech = r.discipline.toLowerCase().includes('mechanical');
              const isControls = r.discipline.toLowerCase().includes('controls');
              ...
              const deptTheme = isPiping
                ...
                : { topBorder: 'border-t-4 border-t-sky-500', bg: 'bg-sky-500/[0.02] dark:bg-sky-500/[0.03]', tagBg: 'bg-sky-500/10 text-sky-800 dark:text-sky-300 border-sky-500/20' };

              return (
                <div
                  key={idx}
                  className={cn(
                    'p-5 rounded-[12px] bg-white dark:bg-[#12151D] border border-black/[0.08] dark:border-white/10 shadow-sm specimen-chamfer space-y-4 flex flex-col justify-between',
                    deptTheme.topBorder,
                    deptTheme.bg
                  )}
                >
                  <div className="space-y-2">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-slate-500 dark:text-zinc-400 mt-1 flex items-center gap-2">
                          ...
                          <span>•</span>
                          <span className="font-semibold text-slate-800 dark:text-zinc-200">Filled: {r.slotsFilled} of {r.slotsNeeded} Slots</span>
                        </div>
                      </div>
                    ...
                    {r.assignedCandidates.length > 0 ? (
                      ...
                      <div className="p-3.5 rounded-[9px] bg-rose-500/[0.04] dark:bg-rose-500/[0.08] border border-rose-500/20 text-xs text-rose-700 dark:text-rose-400 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <AlertTriangle className="w-4 h-4 shrink-0" />
                          <span>Slot vacant. Sourcing required before mobilization.</span>
                        </div>
                        <span className="font-bold font-mono">GAP: 1</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-2 border-t border-black/[0.04] dark:border-white/[0.06] flex items-center justify-between text-[11px] text-slate-500 dark:text-zinc-400">
                    ...
                    <span className="text-emerald-700 dark:text-emerald-400 font-semibold">CSTS-2020 Required</span>
                  </div>
                </div>
              );
            })}
